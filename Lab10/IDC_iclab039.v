...
//===================Gated Clock===================//
wire clk_output, 	clk_calc,	clk_in_data		[63:0];
wire sleep_output,	sleep_calc,	sleep_in_data	[63:0];

assign sleep_output = 	!((current_state == STATE_IDLE) || (current_state == STATE_OUTPUT));
assign sleep_calc =		!((current_state == STATE_IDLE) || (current_state == STATE_CALC));
generate
	for (i=0; i<64; i=i+1) begin
		// assign sleep_in_data[i] = !((current_state == STATE_IDLE) || (in_valid) || ((current_state == STATE_CALC) && (within_subregion[i])));
		assign sleep_in_data[i] = !((current_state == STATE_IDLE) || (in_valid) || (within_subregion[i]));
	end
endgenerate

GATED_OR GATED_OUTPUT(
	.CLOCK(clk),
	.SLEEP_CTRL(cg_en && sleep_output),
	.RST_N(rst_n),
	.CLOCK_GATED(clk_output)
);

GATED_OR GATED_CALC(
	.CLOCK(clk),
	.SLEEP_CTRL(cg_en && sleep_calc),
	.RST_N(rst_n),
	.CLOCK_GATED(clk_calc)
);
...
generate
	for (i=0; i<64; i=i+1) begin
		GATED_OR GATED_CALC(
			.CLOCK(clk),
			.SLEEP_CTRL(cg_en && sleep_in_data[i]),
			.RST_N(rst_n),
			.CLOCK_GATED(clk_in_data[i])
		);
	end
endgenerate
...
generate
	for (i=0; i<64; i=i+1) begin
		always @(posedge clk_in_data[i] or negedge rst_n) begin
